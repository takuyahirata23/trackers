defmodule TrackersWeb.CoreComponents do
  @moduledoc """
  ...
  def modal(assigns) do
    ...
    >
      <div
        id={"#{@id}-bg"}
        class="bg-bg-primary/60 fixed inset-0 transition-opacity"
        aria-hidden="true"
      />
      <div
        ...
        <div class="flex min-h-full items-center justify-center">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              ...
              class="shadow-zinc-700/10 ring-zinc-700/10 relative hidden rounded-2xl bg-bg-secondary p-14 drop-shadow-md ring-1 transition"
            >
              <div class="absolute top-6 right-5">
                <button
                  phx-click={JS.exec("data-cancel", to: "##{@id}")}
                  type="button"
                  class="-m-3 text-white flex-none p-3 hover:opacity-40"
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <select
        id={@id}
        name={@name}
        class="mt-1 block w-full text-bg-primary rounded-md border border-gray-300 bg-white shadow-sm focus:border-zinc-400 focus:ring-0 sm:text-sm"
        multiple={@multiple}
        {@rest}
      >
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(TrackersWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(TrackersWeb.Gettext, "errors", msg, opts)
    end
  end
